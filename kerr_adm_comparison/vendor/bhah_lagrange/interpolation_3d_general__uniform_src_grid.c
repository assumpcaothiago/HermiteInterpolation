...
// pragma disabled (pragma_mode=commented): #pragma GCC optimize("unroll-loops")
...
int interpolation_3d_general__uniform_src_grid(const int n_interp_ghosts, const REAL src_dxx0, const REAL src_dxx1, const REAL src_dxx2,
                                               ...
                                               const int NUM_INTERP_GFS, REAL *restrict src_x0x1x2[], const REAL *restrict src_gf_ptrs[],
  ...
  // pragma disabled (pragma_mode=commented): #pragma omp parallel for
  for (int dst_pt = 0; dst_pt < num_dst_pts; dst_pt++) {
    ...
    // Check if the interpolation stencil goes out of bounds, and adjust indices to prevent memory corruption.
    {
      if ((idx_center_x0 - NinterpGHOSTS < 0) || (idx_center_x0 + NinterpGHOSTS >= src_Nxx_plus_2NGHOSTS0) || (idx_center_x1 - NinterpGHOSTS < 0) ||
        ...
        // pragma disabled (pragma_mode=commented): #pragma omp critical
        {
...
// pragma disabled (pragma_mode=commented): #pragma GCC reset_options // Reset compiler optimizations after the function
